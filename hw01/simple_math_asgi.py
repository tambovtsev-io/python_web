...
class SimpleMathASGIServer(ASGIServerBase):
    async def handle_request(
        ...
        if method != "GET":
            await self.send_error(
                send, HTTPStatus.METHOD_NOT_ALLOWED, f"Method {method} not allowed."
            )
            return

        if path.startswith("/factorial"):
            await self.get_factorial(scope, send)
        elif path.startswith("/fibonacci/"):
            await self.get_fibonacci(path, send)
        elif path == "/mean":
            await self.get_mean(receive, send)
        else:
    ...
    async def get_factorial(
        ...
        if not is_valid:
            await self.send_error(send, status_code, error_message)
            return

        result = math.factorial(n)
        await self.send_json(send, {"result": result})
    ...
    async def get_mean(
        ...
        try:
            ...
            await self.send_error(send, HTTPStatus.UNPROCESSABLE_ENTITY, "Body must be an array of floats.")
            return

        if len(numbers) == 0:
            await self.send_error(send, HTTPStatus.BAD_REQUEST, "Array of floats cannot be empty.")
        else:
